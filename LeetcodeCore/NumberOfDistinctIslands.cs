...
namespace LeetcodeCore
{
    public class NumberOfDistinctIslands
    {
        ...
        public int NumDistinctIslands(int[][] grid)
        {
            ...
            var pointHashSet = new HashSet<Tuple<int, int>>();
            ...
            for (int i = 0; i < m; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    if (grid[i][j] == 1 && !pointHashSet.Contains(new Tuple<int, int>(i,j)))
                    {
        ...
        private void DFSFind(int[][] grid, int m, int n, HashSet<Tuple<int, int>> phs, int i, int j, StringBuilder sb, char direction)
        {
            if (i < 0 || j < 0 || i >= m || j >= n)
                return;
            if (grid[i][j] == 0)
                return;
            if (phs.Contains(new Tuple<int, int>(i, j)))
                return;

            phs.Add(new Tuple<int, int>(i, j));
            sb.Append(direction);
